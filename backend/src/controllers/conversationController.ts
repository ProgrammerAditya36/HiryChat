...
import { getCoversation,createConversation } from '../db/db';
export const newCoversation = async (req: Request, res: Response) => {
    try{
        const {sender, receiver} = req.body;
        let conversation = await getCoversation(sender, receiver);
        if(conversation){
            res.json(conversation);
            return;
        }
        await createConversation(sender, receiver);
        conversation = await getCoversation(sender, receiver);
        res.json(conversation);
    }
    catch(err){
